...
if (isset($_SESSION['update_status']) && $_SESSION['update_status'] === true) {
    ...
    <script>
        Swal.fire({
            ...
            text: 'Foto Anda telah berhasil diperbarui.',
            timer: 3000,// Waktu dalam milidetik (misalnya, 3000 ms = 3 detik)
            showConfirmButton: false // Menyembunyikan tombol "OK
        }).then(function() {
            window.location.href = 'produk.php'; // Redirect ke halaman index setelah menutup SweetAlert
        });
    </script>
    <?php
 ...
 <script>
       Swal.fire({
            ...
            text: 'Foto Anda telah Gagal diperbarui.',
            timer: 3000, // Waktu dalam milidetik (misalnya, 3000 ms = 3 detik)
            showConfirmButton: false // Menyembunyikan tombol "OK"
        }).then(function() {
            window.location.href = 'produk.php'; // Redirect ke halaman index setelah menutup SweetAlert
...
.button-container {
    display: flex;
    justify-content: space-between; /* Untuk mengatur elemen ke kanan dan kiri */
	padding-left: 30px; /* Padding di sebelah kiri */
    padding-right: 27px; /* Padding di sebelah kanan */
}
</style>
    <!--*******************
        Preloader start
					...
					<div class="col-12">
                        <div class="card">
							...
							<div class="button-container">
								<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#Modal">Tambah Produk</button>
								<a href="pindah_ruangan.php" target="_blank" type="button" class="btn btn-success">Memindahkan Ruangan Produk </a>
								<a href="excel_produk.php" target="_blank" type="button" class="btn btn-warning">Export Excel</a>
							</div>
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h4 class="modal-title" id="exampleModalLabel">Input Produk</h4>
										
										</div>
										<div class="modal-body">
										<form action="proses/produk.php" method="post">
											<div class="form-group">
												<h6 for="produk">Kode Produk </h6>
												<?php
												$data = mysql_query("select MAX(kodemax) as kodemax from produk");
												$d = mysql_fetch_array($data);
												$max_kode = $d['kodemax'];
												if (!is_null($max_kode)) {
													// Mengambil angka dari kode maksimum, menambahkannya, dan memformatnya sebagai kode baru
													preg_match("/(\d+)$/", $max_kode, $matches);
													$angka_terakhir = $matches[1] + 1;
													$kode_baru = 'BR' . str_pad($angka_terakhir, 5, '0', STR_PAD_LEFT); // 3 digit, misalnya "001", "002", dst.
												} else {
													// Jika tidak ada data di database, kode pertama akan menjadi "BR001"
													$kode_baru = 'BR00001';
												}
												...
												mkdir($tempdir);
												$teks_qrcode    ="$kode_baru";
												$namafile        ="$kode_baru.png";
												...
												?>
												<input type="text" class="form-control" id="kode_produk" name="kode_produk" value="<?php echo $kode_baru;?>" readonly>
											</div>
											<div class="form-group">
												<h6 for="produk">Nama Produk </h6>
											...
											<div class="form-group">
												...
												<?php
												$sql=mysql_query("SELECT kategori FROM kategori ");
											...
											<div class="form-group">
												<h6 for="ruangan">Ruangan</h6>
												<select class="form-control" id="ruangan" name="ruangan" required>
												<option value="-" disabled selected>Pilih Salah Satu</option>
												<?php
												$sql1=mysql_query("SELECT ruangan FROM ruangan ");
													while ($s=mysql_fetch_array($sql1)) { ?>
													<option value="<?php echo $s['ruangan']; ?>"><?php echo $s['ruangan']; ?></option>
													<?php } ?>
												</select>
											</div>
											<div class="form-group">
												<h6 for="supplier">Supplier</h6>
												<select class="form-control" id="supplier" name="supplier" required>
												<option value="-" disabled selected>Pilih Salah Satu</option>
												<?php
												$sql2=mysql_query("SELECT supplier FROM supplier ");
											...
											<div class="form-group">
												<h6 for="qrcode">QR-Code </h6>
												<a href="qrcode-img/<?php echo $kode_baru ?>.png" target="_blank"><img src="qrcode-img/<?php echo $kode_baru ?>.png" alt="QR Code"> </a> 
											</div>
											<br>
                                    ...
                                    <table id="example3" class="display" style="min-width: 845px">
										...
										while($rw = mysql_fetch_array($sql)){ ?>
                                            <tr>
                                                <td><?php echo $n; ?></td>
												<?php if($rw['foto'] != '' ){ ?>
												<td><a href="fotoproduk/<?php echo $rw['foto']; ?>"  target="_blank"><img width="100" height="100" src="fotoproduk/<?php echo $rw['foto'];?>" alt="Foto <?php echo $rw['nama_produk']; ?>"></a></td>
												<?php } else { ?>
												<td></td>
												<?php } ?>
												...
												<td><?php echo $rw['kategori']; ?></td>
												<td><?php echo $rw['ruangan']; ?></td>
												<td><?php echo $rw['supplier']; ?></td>
												<td><?php echo $rw['status']; ?></td>
                                                <td>
													<div class="d-flex">
														<a href="#" class="btn btn-light shadow btn-xs sharp me-1" data-toggle="modal" data-target="#editfotoModal<?php echo $rw['id'];?>" title="Upload/Edit Foto" ><i class="fa fa-picture-o"></i></a>
														<a href="printqrcode.php?id=<?php echo $rw['kode_produk']; ?>&jum=<?php echo $rw['stok']; ?>" target="_blank" class="btn btn-warning shadow btn-xs sharp me-1" title="Print QR-Code" ><i class="fa fa-print"></i></a>
														<a href="qrcode-img/<?php echo $rw['kode_produk']; ?>.png" target="_blank" class="btn btn-info shadow btn-xs sharp me-1" title="QR-Code"><i class="fa fa-qrcode"></i></a>
														<a href="#" class="btn btn-success shadow btn-xs sharp me-1" data-toggle="modal" data-target="#viewModal<?php echo $rw['id'];?>" title="View"><i class="fa fa-eye"></i></a>
														<a href="#" class="btn btn-primary shadow btn-xs sharp me-1" data-toggle="modal" data-target="#editModal<?php echo $rw['id'];?>" title="Edit"><i class="fa fa-pencil"></i></a>
														<a href="#" class="btn btn-danger shadow btn-xs sharp" data-toggle="modal" data-target="#hapusModal<?php echo $rw['id'];?>" title="Hapus"><i class="fa fa-trash"></i></a>
													</div>												
												</td>												
                                            </tr>
											<!-- Modal -->
											<div class="modal fade" id="editModal<?php echo $rw['id'];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														<div class="modal-header">
														...
														<div class="modal-body">
														<form action="edit/produk.php?id=<?php echo $rw['id'];?>" method="post">
															<input type="hidden" class="form-control" id="kode_produk" name="kode_produk" value="<?php echo $rw['kode_produk'];?>" readonly>
														<div class="form-group">
															<h6 for="produk">Nama Produk </h6>
															<input type="text" class="form-control" id="produk" name="produk" value="<?php echo $rw['nama_produk'];?>">
														</div>
														<div class="form-group">
															<h6 for="kategori">Kategori</h6>
															<select class="default-select wide form-control" name="kategori" id="validationCustom05">
														...
														<div class="form-group">
															...
															<option value="<?php echo $rw['ruangan'];?>"><?php echo $rw['ruangan'];?></option>
															<?php
															$sql1=mysql_query("SELECT ruangan FROM ruangan ");
																while ($s=mysql_fetch_array($sql1)) { ?>
																<option value="<?php echo $s['ruangan']; ?>"><?php echo $s['ruangan']; ?></option>
														...
														<div class="form-group">
															...
															<option value="<?php echo $rw['supplier'];?>"><?php echo $rw['supplier'];?></option>
															<?php
															$sql2=mysql_query("SELECT supplier FROM supplier ");
																while ($s=mysql_fetch_array($sql2)) { ?>
																<option value="<?php echo $s['supplier']; ?>"><?php echo $s['supplier']; ?></option>
														...
														<div class="form-group">
															<h6 for="deskripsi">Deskripsi</h6>
															<textarea class="form-control" id="deskripsi" name="deskripsi" placeholder="Masukkan deskripsi"><?php echo $rw['deskripsi'];?></textarea>
														</div>
														<!-- <div class="form-group">
															<h6 for="qrcode">QR-Code </h6>
															<input type="text" class="form-control" id="qrcode" name="qrcode" placeholder="Masukkan nama qrcode">
														</div> -->
															<br>
															<button type="submit" class="btn btn-primary">Simpan</button>
															<button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
											...
											<!-- Foto Modal -->
											<div class="modal fade" id="editfotoModal<?php echo $rw['id'];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														<div class="modal-header">
															<h4 class="modal-title" id="exampleModalLabel">Upload/Edit Foto Produk</h4>
														
														</div>
														<div class="modal-body">
														<form action="edit/fotoproduk.php?id=<?php echo $rw['id'];?>" method="post" enctype="multipart/form-data">
														<div class="form-group">
															<h6 for="produk">Upload/Edit Foto Produk</h6>
											...
											<!-- Modal View -->
											<div class="modal fade" id="viewModal<?php echo $rw['id'];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														<div class="modal-header">
															<h4 class="modal-title" id="exampleModalLabel">View Produk</h4>
														...
														<div class="form-group">
															<h6 for="produk">Kode Produk </h6>
															<input type="text" class="form-control" id="kode_produk" name="kode_produk" value="<?php echo $rw['kode_produk'];?>" readonly>
														</div>
														<div class="form-group">
															<h6 for="produk">Nama Produk </h6>
															<input type="text" class="form-control" id="produk" name="produk" value="<?php echo $rw['nama_produk'];?>"  readonly>
														</div>
														<div class="form-group">
															<h6 for="produk">Stok </h6>
															<input type="text" class="form-control" id="produk" name="produk" value="<?php echo $rw['stok'];?>"  readonly>
														</div>
														<div class="form-group">
															<h6 for="kategori">Kategori</h6>
															<input type="text" class="form-control" id="produk" name="produk" value="<?php echo $rw['kategori'];?>"  readonly>
														</div>
														<div class="form-group">
															<h6 for="ruangan">Ruangan</h6>
															<input type="text" class="form-control" id="produk" name="produk" value="<?php echo $rw['ruangan'];?>"  readonly>
															
														</div>
														<div class="form-group">
															<h6 for="supplier">Supplier</h6>
															<input type="text" class="form-control" id="produk" name="produk" value="<?php echo $rw['supplier'];?>" readonly>
														</div>
														<div class="form-group">
															<h6 for="deskripsi">Deskripsi</h6>
															<textarea class="form-control" id="deskripsi" name="deskripsi"  readonly><?php echo $rw['deskripsi'];?></textarea>
														</div>
														<div class="form-group">
															<h6 for="qrcode">Foto Produk </h6>
															<a href="fotoproduk/<?php echo $rw['foto']; ?>" target="_blank"><img width="100" height="100" src="fotoproduk/<?php echo $rw['foto']; ?>" alt="<?php echo $rw['nama_produk']; ?>"> </a> 
														</div>
														<!-- <div class="form-group">
															<h6 for="qrcode">QR-Code </h6>
															<input type="text" class="form-control" id="qrcode" name="qrcode" placeholder="Masukkan nama qrcode">
											...
											<div class="modal fade" id="hapusModal<?php echo $rw['id'];?>" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														<div class="modal-header">
														...
														<div class="modal-footer">
															...
															<a href="delete/produk.php?id=<?php echo $rw['id'];?>" class="btn btn-danger" id="hapusButton">Hapus</a>
														</div>
													</div>
												</div>
											</div>
    ...
    ***********************************-->
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
            // Simpan tombol "Hapus" dalam sebuah variabel
            var hapusButton = document.getElementById('hapusButton');
            if (hapusButton) {
                // Klik tombol "Hapus" secara otomatis
                hapusButton.click();
            }
        }
    });
